...
import { searchDrug, suggestDrugs } from '../lib/gemini';
import { db, handleFirestoreError, OperationType } from '../lib/firebase';
import { collection, addDoc, serverTimestamp } from 'firebase/firestore';
import { useAuth } from '../contexts/AuthContext';
import { Loader2, CheckCircle2, AlertCircle, Search as SearchIcon, BookOpen } from 'lucide-react';
...
export default function Search() {
  ...
  const handleSearch = async (e?: React.FormEvent, queryToSearch?: string) => {
    ...
    try {
      const searchResult = await searchDrug(finalQuery);
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl mx-auto space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Drug Search</h1>
        <p className="mt-2 text-slate-500">Search for specific drug dosages based on standard medical formularies like Frank Shann.</p>
      </div>
